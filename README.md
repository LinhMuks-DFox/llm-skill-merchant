...
| `human-ai-research-assistant-kit` | `<suit-for-ai-research-assistant>` | `research-assistant` (8 modes) | AI-Research-Assistant-side kit: research discussion / sparring, concept explaining (teach-me mode: decompose a concept the human doesn't understand, optional concept card), research log writing (incl. the braindump-to-log protocol: structure confirmation → exemplar anchoring → register fidelity → 【AI 补写】 markers → self-check), decision/progress notes, code-agent tasks, technical references, literature-survey fan-out, academic presentation writing. Doc-contract-driven like the code-agent kit: workspace facts live in the workspace-root `RESEARCH-CONTEXT.md` (port to a new workspace with its init procedure). Register-and-accountability discipline, derivation-chain information safety, and the three-layer memory rule built in; heavy document production is delegated to dedicated production suites (evidence layer stays here). Origin: Mac-side `human-ai-research-writing-kit` (verified superset of the takamichi copy; the codebase-snapshot *executor* protocol lives in the code-agent kit, this kit keeps the *requester* side). |
